(async () => {
  ...
  try {
    ...
    let modifiedRules = config.rules.map((rule) => {
     ...
     if (!found) {
            console.log("No existing rule found, will add one.");
            modifiedRules.push(`${aiListRule}${targetProxyGroup}`);
        }
       console.log("Modified Rule:", modifiedRules);

       const modifyConfig = await httpAPI("/v1/profiles/modify", "POST", { rules: modifiedRules});
       if (!modifyConfig) {
  ...
  // prettier-ignore
  function httpAPI(path = "", method = "GET", body = null) {return new Promise((resolve) => {$httpAPI(method, path, body, (result) => {resolve(result);});});}
})();
